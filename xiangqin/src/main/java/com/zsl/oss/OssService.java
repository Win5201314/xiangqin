...
public class OssService {
    ...
    public void beginupload(final Context context, String filename, String path) {
        ...
        if (TextUtils.isEmpty(objectname)) {
            ToastUtil.normalShow(context, "文件名不能为空", true);
            return;
        }
        ...
        put.setProgressCallback(new OSSProgressCallback<PutObjectRequest>() {
            ...
            public void onProgress(PutObjectRequest request, long currentSize, long totalSize) {
                Logger.d("OSS","currentSize: " + currentSize + " totalSize: " + totalSize);
        ...
        OSSAsyncTask task = oss.asyncPutObject(put, new OSSCompletedCallback<PutObjectRequest, PutObjectResult>() {
            ...
            public void onFailure(PutObjectRequest request, ClientException clientExcepion, ServiceException serviceException) {
                // 请求异常
                Logger.d("OSS","UploadFailure");
                ToastUtil.normalShow(context, "UploadFailure", true);
                if (clientExcepion != null) {
                    // 本地异常如网络异常等
                    Logger.e("OSS","UploadFailure：表示向OSS发送请求或解析来自OSS的响应时发生错误。\n" +
                            "  *例如，当网络不可用时，这个异常将被抛出");
                    clientExcepion.printStackTrace();
                }
                if (serviceException != null) {
                    // 服务异常
                    Logger.d("OSS","UploadFailure：表示在OSS服务端发生错误");
